## R Code for Chapter 17. Exploratory Factor Analysis
...
```{r fig.width=4, fig.height=3, message=FALSE}
#Load these packages, along with the boot package.
library(corpcor)
library(GPArotation)
library(psych)
```

#### The R anxiety questionnaire(RAQ) Example
...
Analysis process
 ...
 1. Scan the matrix for correlations > 0.3, 

 2. Look for variables that only have a small number of correlations greater than this value.

 3. Scan the correlation coefficients themselves and look for any greater than .9.

 4. Any are found then you should be aware that a problem could arise because of linearity in the data.
...
Examines whether the population correlation matrix resembles an identity matrix. (17.4.2)
...
```
Conclusion: Bartlett's test is highly significant,X2(253) = 19,334,P< .001,and therefore factor analysis is appropriate.
...
```
Explanation of ther result: Unchanged from the previous factor loading matrix except communalities (=h2 column) and uniquenesses (=u2 column)
...
#upper.tri(): Extracting only the elements above the diagonal (so we discard the diagonal elements and the elements below the diagonal)
...
###Labcoat Leni (Internet addiction Example)
...
**Determinent:should be > 0.00001 for avoiding multicollinearity**
```{r fig.width=4, fig.height=3, message=FALSE}
